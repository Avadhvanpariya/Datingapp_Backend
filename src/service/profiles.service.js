 ...
 */
const getPublicProfileService = async (targetUserId, currentUserId) => {
    ...
    const [followersCount, followingCount, totalPostsCount, isFollowingRecord, currentFollowing] = await Promise.all([
        ...
        currentUserId !== targetUserId ? Follow.find({ follower: currentUserId }).distinct('following') : Promise.resolve([])
    ]);

    const mutualFollowsCount = currentUserId !== targetUserId ? await Follow.countDocuments({ follower: targetUserId, following: { $in: currentFollowing } }) : 0;
