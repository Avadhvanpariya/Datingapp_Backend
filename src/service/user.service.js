...
// --- phone otp login ----------------
const phoneLoginService = async (firebaseToken, name, role) => {
    if (!firebaseToken) {
    ...
    if (user) {
        ...
        if (!user.firebaseUid) { user.firebaseUid = uid; needsSave = true; }

        if (!user.isPhoneVerified) { user.isPhoneVerified = true; needsSave = true; }

        // Standardize phone number format in DB
        if (user.phone !== phone_number) { user.phone = phone_number; needsSave = true; }
...
// ─── Complete Profile ─────────────────────────────────────────────────────────
const completeProfileService = async (body, userId) => {
    const { name, email, phone, role, bio, interests, city, state, country, age, occupation, avatarBase64 } = body;
    ...
    if (email && email.toLowerCase() !== (user.email || '').toLowerCase()) {
        ...
        if (emailExists) {
            throw new AppError('This email is already associated with another account.', 400);
        }
    ...
    if (phone && phone !== user.phone) {
        ...
        if (phoneExists) {
            throw new AppError('This phone number is already associated with another account.', 400);
        }
